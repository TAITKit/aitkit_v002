...
if ($_POST['acceptParemeter'] == 0)
  {
	...
	$praetor->custoupdate('algorithm', array('OwnerPID'=>$_SESSION['user']['PID'], 'AlgorithmTitle'=>$_POST['titleInfo'],'GitHub'=>$_POST['github'],'Abbreviation'=>$_POST['titleAbbre'], 'Input'=>$_POST['inputFormat'], 'Output'=>$_POST['outputFormat'], 'authorName'=>$_POST['authorName'], 'authorUnit'=>$_POST['authorUnit'], 'functionEnglish'=>$_POST['functionEnglish'], 'functionChinese'=>$_POST['functionChinese'], 'functionDescription'=>$_POST['functionDescription'], 'systemEnvironment'=>$_POST['systemEnvironment'], 'package'=>$_POST['package'], 'allowParemeter'=>$_POST['acceptParemeter']), "AID=%s", $_POST['aid']);
  ...
  $sql = "SELECT no FROM link WHERE algorithmID=%d_algorithmID ORDER BY no desc";
            $linkData = $praetor->custosql($sql, array('algorithmID'=>$_POST['aid']));
            if ($linkData)
            {
              $n = 0;
          ...
          $linkWhere->add('algorithmID=%s', $_POST['aid']);//未處理過的資料algorithmID都是-1
  ...
  $sql = "SELECT no FROM tag WHERE algorithmID=%d_algorithmID ORDER BY no desc";
            $tagData = $praetor->custosql($sql, array('algorithmID'=>$_POST['aid']));
            if ($tagData)
            {
              $n = 0;
          ...
          $tagWhere->add('algorithmID=%s', $_POST['aid']);//未處理過的資料algorithmID都是-1
          $tagWhere->add('no=%d', $n);
          $tagWhere->add('ownerID=%d', $_SESSION['user']['PID']);
                $praetor->custoupdate('tag', array('tagName'=>$_POST['tagName'.$n], 'abbreviation'=>$_POST['titleAbbre'], 'ownerID'=>$_SESSION['user']['PID']), "%l", $tagWhere);
  ...
  $sql = "SELECT no FROM dataSet WHERE algorithmID=%d_algorithmID ORDER BY no desc";
          ...
          $dataSetWhere->add('algorithmID=%s', $_POST['aid']);//未處理過的資料algorithmID都是-1
...
//    {
//       $praetor->custoinsert('tag', array('tagName'=>$tag, 'algorithmID'=>$_POST['aid'], 'abbreviation'=>$_POST['titleAbbre'], 'ownerID'=>$_SESSION['user']['PID']));
//       $tag = '';
//    }
//    if ($n == $tagLenth - 1)
//    {
